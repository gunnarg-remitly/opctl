...
// IsAuthError returns true if data resolution failed because of auth issues
func (err ErrDataResolution) IsAuthError() bool {
	for _, err := range err.errs {
		if errors.Is(err, model.ErrDataProviderAuthorization{}) ||
			errors.Is(err, model.ErrDataProviderAuthentication{}) {
			return true
		}
	}
	return false
