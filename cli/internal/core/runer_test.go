...
import (
	...
	datahandleFakes "github.com/opctl/opctl/sdks/go/model/fakes"
...
var _ = Context("Runer", func() {
	...
	Context("Run", func() {
		It("dataResolver.Resolve call", func() {
			...
			fakeDataResolver := new(dataresolverFakes.FakeDataResolver)
			...
			/* act */
			err := objectUnderTest.Run(providedCtx, providedOpRef, &RunOpts{}, false)
		...
		It("opfile.GetContent call", func() {
			...
			fakeOpHandle := new(datahandleFakes.FakeDataHandle)
			...
			/* act */
			err := objectUnderTest.Run(context.TODO(), "", &RunOpts{}, false)
		...
		Context("opfile.Get doesn't error", func() {
			...
			It("cliParamSatisfier yml file failure", func() {
				...
				/* act */
				err := objectUnderTest.Run(context.TODO(), "", &RunOpts{ArgFile: "argfile"}, false)
			...
			It("should call core.StartOp w/ expected args", func() {
				...
				// stub w/ closed channel so test doesn't wait for events indefinitely
				...
				/* act */
				objectUnderTest.Run(providedContext, "", &RunOpts{}, false)
			...
			Context("core.StartOp doesn't error", func() {
				...
				Context("eventStream doesn't error", func() {
					Context("event channel closes", func() {
						It("should return expected error", func() {
							/* arrange */
							dummyOpDataHandle := getDummyOpDataHandle()
